...
local camps_complete = 0;
	...
	if camps_incomplete == 0 then
		timer:stop()
		mission_stage = "Mission Complete";
		ways():enable();
		instead.autosave();
	else
		timer:stop()
		mission_stage = "Remained another bases: "..camps_incomplete;
	end;
end;
	...
	for i = 1, 16 do
		for j = 1, 16 do
			if map[i][j] == "P" then
				ship_pos_x = j;
				ship_pos_y = i;
				mission_stage = "Runway^coordinates: "..ship_pos_x.." "..ship_pos_y;
			end;
		end;
	end;
end;
...
stat {
	nam = "inf",
	disp = "Command Control Reference:^",
...
menu {
	...
	dsc = "{Control Basics}",
	act = function()
		return p ("Basic Commands:^^".."["..'{$fnt psst, "white"|↑}'.."] ("..'{$fnt psst, "white"|+}'..") - flight to the next square at the rate^^".."["..'{$fnt psst, "white"|←}'.."] ("..'{$fnt psst, "white"|<}'..") - turn left in place^^".."["..'{$fnt psst, "white"|→}'.."] ("..'{$fnt psst, "white"|>}'..") - turn right in place^^".."["..'{$fnt psst, "magenta"|*}'.."] ("..'{$fnt psst, "white"|Space}'..") - cargo discharge^^".."["..'{$fnt psst, "red"|R}'.."] ("..'{$fnt psst, "white"|Enter}'..") - run program^^".."["..'{$fnt psst, "cadetblue"|D}'.."] ("..'{$fnt psst, "white"|Backspace}'..") - removing command^^");
	end,
}:disable();
...
menu {
	...
	dsc = "{Procedures}",
	act = function()
		return p ([[You can combine command sequences into procedures so that you do not specify the same sequence multiple times.]].."^^"..[[The procedure is specified in the column ]].."["..'{$fnt psst, "red"|0}'.."], ["..'{$fnt psst, "yellow"|1}'.."] или ["..'{$fnt psst, "blue"|2}'.."], and in the right place of the program is enough to press the "..'{$fnt psst, "white"|0}'..", "..'{$fnt psst, "white"|1}'.." or "..'{$fnt psst, "white"|2}'.." key.^^");
	end,
}:disable();
...
menu {
	...
	dsc = "{Loops}",
	act = function()
		return p ([[Another way to shorten a record is applied to chains of the same command.]].."^^"..[[You must specify how many times to repeat the command. This technique is called a cycle. To do this, use the keys ]].."["..'{$fnt psst, "lightgreen"|3}'.."], ".."["..'{$fnt psst, "lightgreen"|4}'.."], ".."["..'{$fnt psst, "lightgreen"|5}'.."], ".."["..'{$fnt psst, "lightgreen"|6}'.."], ^".."["..'{$fnt psst, "lightgreen"|7}'.."], ".."["..'{$fnt psst, "lightgreen"|8}'.."], ".."["..'{$fnt psst, "lightgreen"|9}'.."].^^"..[[Procedures can also be repeated in a loop.]]);
	end,
}:disable();
...
menu {
	...
	act = function(s)
		s.disp = "Economical <<^";
		_"anim_speed_x2".disp = "Full^";
		_"anim_speed_x4".disp = "Afterburn^";
--		pn ("The economic speed of the ship is established");
		animation_repeats = 24;
		animation_step = 4;
--		repeat_local = 24;
--		step_local = 4;
...
menu {
	...
	act = function(s)
		s.disp = "Full <<^";
		_"anim_speed_x1".disp = "Economical^";
		_"anim_speed_x4".disp = "Afterburn^";
--		pn ("The full speed of the ship is established");
...
menu {
	...
	act = function(s)
		...
		_"anim_speed_x1".disp = "Economical^";
		_"anim_speed_x2".disp = "Full^";
--		pn ("The afterburned rush speed of the ship is established");
		animation_repeats = 6;
		animation_step = 16;
--		repeat_local = 6;
--		step_local = 16;
...
room {
	...
	dsc = function(s)
		...
		p ([[Remake of the game "Shturman" by Oleg Shamshura]]..s.postf);
	end,
}
...
room {
	...
	disp = "***Briefing***",
	{
		dsc_txt = "^"..[[You are a cadet of the flying academy. Your task - to master the laying of the course with the help of the newest cybernetics complex of navigation "Autopilot of the Higher Sort. Abbreviated - A0HS.]].."^^"..[[Lay the course so that the ship could avoid troubles and deliver cargo to all bases. The ship always starts from the landing site.]],
		letter_mark = 0,
		disp_text = [[]],
	},
	enter = function(s)
...
room {
	...
	title = "***Control***",
	{
		dsc_txt = "^"..[[The behavior of the ship is set by the program.
	It consists of commands and is written^in a stack ]].."["..'{$fnt psst, "magenta"|P}'.."]^^".."Basic commands:^^".."["..'{$fnt psst, "white"|↑}'.."] ("..'{$fnt psst, "white"|+}'..") - flight to the next square at the exchange rate^^".."["..'{$fnt psst, "white"|←}'.."] ("..'{$fnt psst, "white"|<}'..") - rotate in place to the left^^".."["..'{$fnt psst, "white"|→}'.."] ("..'{$fnt psst, "white"|>}'..") - rotate in place to the right^^".."["..'{$fnt psst, "magenta"|*}'.."] ("..'{$fnt psst, "white"|Space}'..") - cargo discharge^^".."["..'{$fnt psst, "red"|R}'.."] ("..'{$fnt psst, "white"|Enter}'..") - run program^^".."["..'{$fnt psst, "cadetblue"|D}'.."] ("..'{$fnt psst, "white"|Backspace}'..") - remove command",
		letter_mark = 0,
		disp_text = [[]],
	},
	enter = function(s)
...
room {
	...
	disp = "Lesson 2",
	{
		dsc_txt = "^"..[[You can combine command sequences into procedures so that you do not specify the same sequence multiple times.]].."^^"..[[The procedure is specified in the column ]].."["..'{$fnt psst, "red"|0}'.."], ".."["..'{$fnt psst, "yellow"|1}'.."] or ".."["..'{$fnt psst, "blue"|2}'.."], and in the right place of the program is enough to press the {$fnt psst, 'white'|0}, {$fnt psst, 'white'|1} or {$fnt psst, 'white'|2} key.",
		letter_mark = 0,
		disp_text = [[]],
	},
	title = "***Procedures***",
...
room {
	...
	disp = "Lesson 3",
	{
		dsc_txt = "^"..[[Another way to shorten the entry applies to chains of identical commands.]].."^^"..[[You must specify how many times to repeat the command. This technique is called a loop. To do this, use the keys ]].."["..'{$fnt psst, "lightgreen"|3}'.."], ".."["..'{$fnt psst, "lightgreen"|4}'.."], ".."["..'{$fnt psst, "lightgreen"|5}'.."], ".."["..'{$fnt psst, "lightgreen"|6}'.."], ^".."["..'{$fnt psst, "lightgreen"|7}'.."], ".."["..'{$fnt psst, "lightgreen"|8}'.."], ".."["..'{$fnt psst, "lightgreen"|9}'.."].",
		letter_mark = 0,
		disp_text = [[]],
	},
	title = "***Loops***",
...
room {
	...
	disp = "Lesson 4",
	{
		dsc_txt = "^"..[[Procedures can also be repeated in a loop.]],
		letter_mark = 0,
		disp_text = [[]],
	},
	title = "***Loops of procedures***",
	enter = function(s)
		timer:set(30);
	end;
	pic = function()
...
room {
	...
	title = "",
	{
		...
		info_x = 0,
		info_y = -40,
		info =  "Congratulations, you will be good!",
		postf = "",
		time_line = 0,
	},
	timer = function(s)
